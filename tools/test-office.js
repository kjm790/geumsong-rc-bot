...
assert.strictEqual(say(222, '/recruitcheck')[0], 'RECRUIT:check', '명단을 고칠 수 있는 임원은 입력 누락 점검도 가능');
assert.ok(say(222, '/settings')[0].indexOf('권한이 필요') !== -1, '회장은 관리자 명령 불가');
assert.ok(say(555, '/recruitcheck')[0].indexOf('권한이 필요') !== -1, '참관은 점검 불가');
